...
create_footer <- function (source_name, logo_image_path) {
  ...
  footer <- grid::grobTree(#grid::linesGrob(x = grid::unit(c(0, 1), "npc"), y = grid::unit(1.1, "npc")),
                           #ggpubr::text_grob(paste0("Source: ", source_name),
                           #grid::grid.text(paste0("Source: ", source_name),
                           #          gp=gpar(family = "Roboto Regular", cex=5))
...
finalise_plot <- function(plot_name,
                          ...
                          width_pixels=640,
                          height_pixels=450 #,
                          #  logo_image_path = file.path(system.file(
                          #   "data", package = 'bbplot'),"placeholder.png")
) {
  ...
  plot_grid <- ggpubr::ggarrange(plot_left_aligned, footer,
                                 ncol = 1, nrow = 2,
                                 heights = c(1, 0.07/(height_pixels/450)))
